...
void main() {
  ...
  if (sum_broken == 0) { // k == 0; regular triangle
    ...
    normal = normalize(cross(teCartesianPosition[1] - teCartesianPosition[0], teCartesianPosition[2] - teCartesianPosition[0]));
    EmitSphericalVertex(teCartesianPosition[0], sphericalPosition[0], dot(teCartesianPosition[0], normal) > 0 ? -normal : normal);
    EmitSphericalVertex(teCartesianPosition[1], sphericalPosition[1], dot(teCartesianPosition[1], normal) > 0 ? -normal : normal);
    EmitSphericalVertex(teCartesianPosition[2], sphericalPosition[2], dot(teCartesianPosition[2], normal) > 0 ? -normal : normal);
    ...
    if (sum_broken == 2) { // the triangle crosses the hind meridian
      ...
      EmitSphericalVertex(p_a, s_ax, normal); // a'
      EmitSphericalVertex(p_c, s_c,  normal); // c
      EmitSphericalVertex(p_b, s_bm, normal); // b or b'
      EndPrimitive();
      EmitSphericalVertex(p_a, s_a,  normal); // a
      EmitSphericalVertex(p_c, s_cx, normal); // c'
      EmitSphericalVertex(p_b, s_bp, normal); // b or b'
      EndPrimitive();
      ...
      EmitSphericalVertex(p_b   , s_b    , normal); // b
      EndPrimitive();
